...
router.post('/api/suppliers', async (req, res) => {
  ...
  const [dup] = await pool.query('SELECT id FROM suppliers WHERE code = ?', [code])
  if (dup.length > 0) return res.status(409).json({ error: '编码已存在' })

  const [result] = await pool.query(
    'INSERT INTO suppliers (code, name, contact, phone, address, remark) VALUES (?,?,?,?,?,?)',
...
router.put('/api/suppliers/:id', async (req, res) => {
  ...
  const [existing] = await pool.query('SELECT id FROM suppliers WHERE id = ? AND status = 1', [req.params.id])
  if (existing.length === 0) return res.status(404).json({ error: '供应商不存在' })
  ...
  if (Object.keys(updates).length > 0) {
    await pool.query('UPDATE suppliers SET ? WHERE id = ?', [updates, req.params.id])
  }
  res.json({ ok: true })
})
...
router.delete('/api/suppliers/:id', async (req, res) => {
  ...
  const [existing] = await pool.query('SELECT id FROM suppliers WHERE id = ? AND status = 1', [req.params.id])
  if (existing.length === 0) return res.status(404).json({ error: '供应商不存在' })
  await pool.query('UPDATE suppliers SET status = 0 WHERE id = ?', [req.params.id])
  res.json({ ok: true })
})
